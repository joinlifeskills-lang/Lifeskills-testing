  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-4">
        {messages
          .filter((msg) => !deletedMsgIds.has(msg.id))
          .map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`group flex flex-col ${isMember ? "items-end" : "items-start"}`}
              >
                <div className={`relative flex items-center gap-1.5 ${isMember ? "flex-row-reverse" : "flex-row"}`}>
                  {msg.type === "voice" ? (
                  ...
                  <button
                    ...
                    className="hidden group-hover:flex items-center justify-center p-1 rounded-md hover:bg-red-50 text-neutral-300 hover:text-red-500 transition-colors shrink-0"
